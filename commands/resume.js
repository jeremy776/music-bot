...
module.exports.run = async(client, msg, args) => {
  ...
  if(!queue.isPause) return msg.channel.send(new MessageEmbed().setDescription("Musik sedang tidak di pause").setColor("#7289da").setTimestamp());
  
  queue.connection.dispatcher.resume();
  queue.isPause = false;
  
  return msg.channel.send(new MessageEmbed().setDescription("Musik berhasil di lanjutkan").setColor("#7289DA").setTimestamp());
};
